    ...
    <>
      ...
      <main style={{ paddingTop: '68px' }}>
        <style>{`
          ...
          .article-body h3 { font-family: var(--font-body); font-size: clamp(18px, 2.2vw, 21px); font-weight: 800; color: #061A3A; letter-spacing: -0.01em; line-height: 1.25; margin: 30px 0 12px; }
          .article-body h4 { font-family: var(--font-body); font-size: 16px; font-weight: 800; color: #061A3A; line-height: 1.3; margin: 24px 0 10px; }
          ...
          .article-body blockquote p { font-size: 15px; color: #1468E8; font-weight: 600; margin: 0; line-height: 1.65; }
        ...
        <section style={{ background: '#fff', padding: 'clamp(48px, 6vw, 72px) 24px' }}>
          <div style={{ maxWidth: '940px', margin: '0 auto', display: 'grid', gridTemplateColumns: '230px 1fr', gap: '56px', alignItems: 'start' }} className="article-layout">
            ...
            {/* Body */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} style={{ minWidth: 0 }}>
              ...
              {headings.length > 0 && (
                <>
                  ...
                  <nav style={{ borderLeft: '2px solid #ECEEF1', marginBottom: '28px' }}>
                    {headings.map(h => (
                      <a key={h.id} href={`#${h.id}`} className={`toc-link${activeId === h.id ? ' active' : ''}`}>{h.text}</a>
                    ))}
                  </nav>
                </>
              )}
