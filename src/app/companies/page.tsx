              ...
              {STEPS.map((s, i) => (
                <motion.div key={s.num} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} style={{ display: 'flex', gap: '24px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, width: '52px' }}>
                    <div style={{ width: '52px', height: '52px', borderRadius: '50%', background: i === 0 ? '#D8F950' : 'rgba(255,255,255,0.08)', border: `2px solid ${i === 0 ? '#D8F950' : 'rgba(255,255,255,0.15)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <s.icon size={20} color={i === 0 ? '#041635' : '#D8F950'} strokeWidth={2} />
                    </div>
                    {i < STEPS.length - 1 && (
              ...
              {FEATURE_TABS.map((t, i) => activeTab === i && (
                <motion.div key={t.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }}>
                  ...
                  >
                    ...
                    {/* Constrained: show top portion only */}
                    <div style={{ maxHeight: '380px', overflow: 'hidden', position: 'relative' }}>
