                      ...
                      {currentGroup.map(({ id, icon: Icon, label, desc }) => {
                        ...
                        return (
                          <button key={id} onClick={() => setSelectedType(id)} className={active ? '' : 'gs-type-btn'}
                            style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '13px 14px', borderRadius: '14px', border: active ? '2px solid #D8F950' : '2px solid #ECEEF1', background: active ? '#041635' : '#fff', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s', width: '100%' }}>
                            <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: active ? 'rgba(216,249,80,0.15)' : '#F0F2F5', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background 0.15s' }}>
