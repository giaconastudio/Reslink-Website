    ...
    <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', border: '1px solid #EEEEF0', boxShadow: '0 8px 32px rgba(11,20,55,0.08)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
        {[
          ...
          <div key={s.label} style={{ background: '#F7F8FA', borderRadius: '10px', padding: '14px' }}>
            <p style={{ fontWeight: 700, fontSize: '22px', color: '#041635' }}>{s.value}</p>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '5px', height: '52px' }}>
        {[30, 50, 38, 68, 52, 88, 62].map((h, i) => (
          <div key={i} style={{ flex: 1, borderRadius: '3px 3px 0 0', height: `${h}%`, background: i === 5 ? '#D8F950' : '#EEEEF0' }} />
        ))}
      </div>
  ...
  return (
    <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', border: '1px solid #EEEEF0', boxShadow: '0 8px 32px rgba(11,20,55,0.08)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {[
          { name: 'Oliver Stone', role: 'Supply Chain · New York', initials: 'OS', color: '#041635' },
          ...
          <div key={c.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#F7F8FA', borderRadius: '10px', padding: '12px 14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div>
                <p style={{ fontSize: '14px', fontWeight: 600, color: '#041635' }}>{c.name}</p>
                <p style={{ fontSize: '12px', color: '#9A9FA8' }}>{c.role}</p>
              </div>
            </div>
            <button style={{ background: '#EDFDB0', color: '#041635', fontSize: '12px', fontWeight: 600, padding: '6px 14px', borderRadius: '8px', border: 'none', cursor: 'pointer' }}>View</button>
          </div>
        ))}
      </div>
    </div>
          ...
          {sections.map((s) => (
            ...
            >
              <div style={{ direction: 'ltr' }}>
                <p style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#D8F950', marginBottom: '14px' }}>{s.label}</p>
                <h2 style={{ fontSize: 'clamp(28px, 3vw, 40px)', fontWeight: 800, letterSpacing: '-0.025em', color: '#041635', lineHeight: 1.15, marginBottom: '16px' }}>{s.title}</h2>
